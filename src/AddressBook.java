...
public class AddressBook {
	...
	public void addContactAddress(ContactAddress contactAddress) {
		if (contactAddresses.stream().anyMatch(existingContact -> existingContact.equals(contactAddress))) {
	...
	public void showContactsByCity(String city) {
		cityDictionary.getOrDefault(city, Collections.emptyList()).forEach(ContactAddress::showInfo);
	}

	public void showContactsByState(String state) {
		stateDictionary.getOrDefault(state, Collections.emptyList()).forEach(ContactAddress::showInfo);
	}
	...
	public Map<String, Long> countContactsByCity() {
		return contactAddresses.stream().collect(Collectors.groupingBy(ContactAddress::getCity, Collectors.counting()));
	}
	...
	public void displayContactCountsByCity() {
		...
		if (countsByCity.isEmpty()) {
			...
			countsByCity
					.forEach((city, count) -> System.out.println("City: " + city + ", Number of Contacts: " + count));
		}
	}
	...
	public void displayContactCountsByState() {
		...
		if (countsByState.isEmpty()) {
			...
			countsByState.forEach(
					(state, count) -> System.out.println("State: " + state + ", Number of Contacts: " + count));
		}
	}
	...
	public void displayContactsSortedByCity() {
		List<ContactAddress> sortedByCity = contactAddresses.stream()
				.sorted(Comparator.comparing(ContactAddress::getCity)).collect(Collectors.toList());
	...
	public void displayContactsSortedByState() {
		List<ContactAddress> sortedByState = contactAddresses.stream()
				.sorted(Comparator.comparing(ContactAddress::getState)).collect(Collectors.toList());
	...
	public void displayContactsSortedByZipCode() {
		List<ContactAddress> sortedByZipCode = contactAddresses.stream()
				.sorted(Comparator.comparing(ContactAddress::getZip)).collect(Collectors.toList());
	...
	public void saveContactsToCSV(String filename) {
		try (CSVWriter writer = new CSVWriter(new FileWriter(filename))) {
			...
			for (ContactAddress contact : contactAddresses) {
				String[] row = { contact.getFirstName(), contact.getLastName(), contact.getAddress(), contact.getCity(),
	...
	public void loadConatctsFromCSV(String filename) {
		try (CSVReader reader = new CSVReader(new FileReader(filename))) {
			...
			for (int i = 1; i < allData.size(); i++) { // Assuming the first row is headers
				...
				ContactAddress contact = new ContactAddress(row[0], row[1], row[2], row[3], row[4], row[5], row[6],
						row[7]);
	...
	public void saveContactsToJSON(String filename) {
		ObjectMapper mapper = new ObjectMapper();
		try {
			mapper.writeValue(new File(filename), contactAddresses);
			System.out.println("Contacts have been saved to " + filename);
		} catch (IOException e) {
	...
	public void loadContactsFromJSON(String filename) {
		ObjectMapper mapper = new ObjectMapper();
		try {
			contactAddresses = mapper.readValue(new File(filename), new TypeReference<List<ContactAddress>>() {
			});
			System.out.println("Contacts have been loaded from " + filename);
			System.out.println("Loaded Contacts:");
